...
exports.addAttachments = async (files) => {
    if (!files || files.length === 0) return;
    await db.query('INSERT INTO attachments (news_id, file_path, file_type, original_name) VALUES ?', [files]);
};
...
exports.deleteByIds = async (ids) => {
    ...
    const idArray = Array.isArray(ids) ? ids : [ids];
    if (idArray.length === 0) return;
    await db.query('DELETE FROM attachments WHERE id IN (?)', [idArray]);
};
